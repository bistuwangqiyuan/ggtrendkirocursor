...
All of that still runs *inside* the thing it watches, and the 44-hour freeze showed why that is not enough: the pipeline could neither publish nor report why, and the site cannot deploy a fix when Netlify refuses builds (an exhausted credit balance blocks deploys while the running site keeps serving). So the same loop also runs from outside, in [`.github/workflows/publish-snapshots.yml`](.github/workflows/publish-snapshots.yml): 25 minutes after each batch it drives the rebuild endpoint and then asserts the ages readers see (`npm run snapshots:check`), failing the run — which mails the owner — when anything is older than four hours. It needs one repository secret, `SNAPSHOT_REBUILD_SECRET`, holding the site's `ADMIN_SECRET` or `CRON_SECRET`.
